...
func InitalizeSuperNode(id string, ip string) {
	...
	NodeNetwork = Network{Kademlia{NewRoutingTable(me), k, make(map[string][]byte), make(map[string]chan string)}, alpha, make(chan Message, alpha), make(map[string]chan bool)}
